...
- (PlayerState *)fetchState {
    ...
    "\n"
    "    try\n"
    "        set albumArt to do JavaScript \"(()=>{ const m=(navigator.mediaSession&&navigator.mediaSession.metadata)?navigator.mediaSession.metadata:null; if(m&&m.artwork&&m.artwork.length){ const x=m.artwork[m.artwork.length-1]; if(x&&x.src) return String(x.src); } const norm=(u)=>{ const s=String(u||'').trim(); if(!s) return ''; if(s.startsWith('blob:')) return ''; if(s.startsWith('//')) return 'https:'+s; if(s.startsWith('/')) return 'https://music.youtube.com'+s; if(s.startsWith('http://')) return 'https://'+s.slice(7); return s; }; const pick=(img)=>{ if(!img) return ''; let u=''; if(img.currentSrc) u=String(img.currentSrc); else if(img.src) u=String(img.src); else { const srcset=String(img.getAttribute('srcset')||'').trim(); if(srcset){ const last=srcset.split(',').map(s=>s.trim()).filter(Boolean).pop()||''; u=last.split(' ')[0]||''; } } return norm(u); }; const selectors=['ytmusic-player-bar img.image','ytmusic-player-bar #song-image img','ytmusic-player-page #song-image img','ytmusic-player-page img#img','ytmusic-player-page img.cover','ytmusic-player-page img']; for (const sel of selectors){ const n=document.querySelector(sel); const u=pick(n); if(u) return u; } const candidates=[...document.querySelectorAll('img')].map((img)=>pick(img)).filter(Boolean); const best=candidates.find((u)=>/googleusercontent|ytimg|gstatic/i.test(u))||candidates[0]||''; return best; })()\" in targetTab\n"
    "    end try\n"
    "\n"
    "    try\n"
    ...
    "\n"
    "    try\n"
    "        set lyricsText to do JavaScript \"(()=>{ const sels=['#lyrics #description','ytmusic-player-page #lyrics #description','ytmusic-tab-renderer[tab-id=\\\\\\\"MUSIC_PAGE_TYPE_TRACK_LYRICS\\\\\\\"] #description','ytmusic-tab-renderer[tab-id=\\\\\\\"lyrics\\\\\\\"] #description','ytmusic-tab-renderer[tab-id*=\\\\\\\"LYRICS\\\\\\\"] #description','ytmusic-tab-renderer[tab-id*=\\\\\\\"lyrics\\\\\\\"] #description','ytmusic-description-shelf-renderer #description']; const norm=(v)=>String(v||'').replace(/\\\\r/g,'').replace(/\\\\u00a0/g,' ').replace(/\\\\n{3,}/g,'\\\\n\\\\n').trim(); const bad=(s)=>!s||s.length<8||/lyrics are not available|no lyrics|가사를 찾을 수 없습니다|가사 정보가 없습니다/i.test(s); const readBest=()=>{ let best=''; for (const sel of sels){ const nodes=document.querySelectorAll(sel); for (const n of nodes){ const t=norm(n.innerText||n.textContent); if(!bad(t) && t.length>best.length) best=t; } } if(!best){ const shelves=document.querySelectorAll('ytmusic-description-shelf-renderer'); for (const sh of shelves){ const t=norm(sh.innerText||sh.textContent); if(!bad(t) && t.length>best.length) best=t; } } return best; }; let best=readBest(); if(!best){ const now=Date.now(); const last=Number(window.__ytmLyricsAutoOpenTs||0); if(now-last>5000){ const tabs=[...document.querySelectorAll('ytmusic-player-page [role=\\\\\\\"tab\\\\\\\"], ytmusic-player-page tp-yt-paper-tab, ytmusic-player-page .tab-title')]; const target=tabs.find((el)=>{ const id=String(el.getAttribute('tab-id')||'').toLowerCase(); const txt=String(norm(el.innerText||el.textContent)||'').toLowerCase(); const aria=String(el.getAttribute('aria-label')||'').toLowerCase(); return id.includes('lyric')||txt.includes('lyrics')||txt.includes('가사')||aria.includes('lyric')||aria.includes('가사'); }); if(target){ const selected=String(target.getAttribute('aria-selected')||'').toLowerCase()==='true'||target.classList.contains('selected')||target.hasAttribute('iron-selected'); if(!selected){ window.__ytmLyricsAutoOpenTs=now; try{ target.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true})); }catch(_){ } try{ if(typeof target.click==='function') target.click(); }catch(_){ } } } } } best=readBest(); return best.length>12000?best.slice(0,12000):best; })()\" in targetTab\n"
    "    end try\n"
    "\n"
